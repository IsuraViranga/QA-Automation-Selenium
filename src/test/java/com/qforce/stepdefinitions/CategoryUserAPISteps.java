...
public class CategoryUserAPISteps {
    ...
    // @Then("Category should not be created in the system")
    // public void category_should_not_be_created_in_the_system() {
    //     logger.info("Step: Verifying category was not created");

    //     // Get current category count using admin token
    //     String adminUsername = ConfigReader.getAdminUsername();
    //     String adminPassword = ConfigReader.getAdminPassword();
    ...
    //     List<Object> categories = categoriesResponse.jsonPath().getList("$");
    //     int currentCount = categories != null ? categories.size() : 0;

    //     Assert.assertEquals(currentCount, categoryCountBeforeTest,
    //             "Category count should remain unchanged. Before: " + categoryCountBeforeTest + ", After: " + currentCount);

    //     logger.info("Verified: Category was not created (count unchanged: {})", currentCount);
    // }

    //@Then("Category should not be deleted from the system")
    // public void category_should_not_be_deleted_from_the_system() {
    //     logger.info("Step: Verifying category was not deleted");
    ...
    //     logger.info("Verified: Category was not deleted (count unchanged: {})", currentCount);
    // }
}
